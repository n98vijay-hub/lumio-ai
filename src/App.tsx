  ...
  useEffect(() => {
    ...
    style.textContent = `
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .card-hover:hover { transform: translateY(-2px); border-color: #ff6b4a88 !important; transition: all 0.2s ease; }
      textarea:focus, input:focus { border-color: #ff6b4a !important; box-shadow: 0 0 0 3px #ff6b4a22 !important; outline: none; }
    `;
...
const T = {
  ...
  cardHover: "#201c3a",
  border: "#2a2545",
  borderHover: "#ff6b4a55",
  primary: "#ff6b4a",
  primaryDark: "#e85535",
  primaryGlow: "#ff6b4a33",
  accent: "#4affd4",
  accentGlow: "#4affd422",
  gold: "#ffd166",
  text: "#f0ecff",
...
// ─── HELPERS ──────────────────────────────────────────────
const callClaude = async (userMessage, systemPrompt, maxTokens = 1200) => {
...
const Btn = ({ children, onClick, variant = "primary", style = {}, disabled = false }) => {
  const styles = {
    ...
    accent: { background: `linear-gradient(135deg, ${T.accent}, #22d3c0)`, color: T.bg, boxShadow: `0 4px 20px ${T.accentGlow}` },
  };
  return (
...
const Card = ({ children, style = {}, className = "" }) => (
  <div className={`card-hover ${className}`} style={{
    ...
    padding: "24px", transition: "all 0.2s", ...style
  }}>{children}</div>
);
...
const Input = ({ value, onChange, placeholder, type = "text", style = {}, onKeyDown }) => (
  <input type={type} value={value} onChange={onChange} placeholder={placeholder} onKeyDown={onKeyDown}
    style={{
      width: "100%", padding: "14px 18px", borderRadius: "14px", border: `1.5px solid ${T.border}`,
      background: T.surface, color: T.text, fontSize: "15px", fontFamily: T.font,
...
const TextArea = ({ value, onChange, placeholder, rows = 5 }) => (
  <textarea value={value} onChange={onChange} placeholder={placeholder} rows={rows}
    style={{
      width: "100%", padding: "14px 18px", borderRadius: "14px", border: `1.5px solid ${T.border}`,
      background: T.surface, color: T.text, fontSize: "15px", fontFamily: T.font,
      resize: "vertical", transition: "all 0.2s", lineHeight: "1.6"
    }} />
);

const Badge = ({ children, color = T.primary }) => (
  <span style={{
    background: `${color}22`, color, borderRadius: "20px", padding: "4px 14px",
    fontSize: "12px", fontWeight: "700", letterSpacing: "0.5px", border: `1px solid ${color}44`
  }}>{children}</span>
);
...
export default function App() {
  ...
  // Chat
  const [msgs, setMsgs] = useState([{ role: "ai", text: "Hey! I'm Lumio ✨ I can help you learn anything, build a career roadmap, explain tough topics, or quiz you. What's on your mind?" }]);
        ...
        `Create a detailed career learning roadmap. Return ONLY valid JSON (no markdown):
{
  ...
  "weeks": [
    {"week": "Week 1-2", "topic": "Topic name", "tasks": ["task1","task2","task3"]}
  ],
...
Include 6 week ranges, 3 projects, 5 skills, 3 interview questions.`, 1500);
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      const reply = await callClaude(`${history}\nStudent: ${msg}`,
        `You are Lumio, a brilliant, friendly AI learning coach. You help with any subject, career advice, study tips, and skill building. Be warm, clear, encouraging. Use simple language everyone can understand. Add relevant emojis. Keep responses helpful but not too long.`, 800);
  ...
  if (screen === "login" || screen === "signup") {
    ...
    return (
      <div style={{ fontFamily: T.font, background: T.bg, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px", position: "relative", overflow: "hidden" }}>
        ...
        <div className="fadeUp" style={{ width: "100%", maxWidth: "420px", background: T.card, borderRadius: "28px", padding: "44px 40px", border: `1px solid ${T.border}`, boxShadow: `0 30px 80px rgba(0,0,0,0.5), 0 0 0 1px ${T.border}` }}>
          {/* Logo */}
          ...
          <div style={{ textAlign: "center", marginTop: "24px", fontSize: "14px", color: T.muted }}>
            ...
            <span style={{ color: T.primary, fontWeight: "700", cursor: "pointer" }}
  ...
  if (screen === "pricing") {
    return (
      <div style={{ fontFamily: T.font, background: T.bg, minHeight: "100vh", padding: "52px 20px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: "1020px", margin: "0 auto" }}>
          <div className="fadeUp" style={{ textAlign: "center", marginBottom: "56px" }}>
            ...
            <div style={{ color: T.muted, fontSize: "16px" }}>Start free. Upgrade when you're ready. Cancel anytime.</div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "20px" }}>
            {PLANS.map((p, i) => (
              <div key={p.name} className="card-hover" style={{
                ...
                borderRadius: "24px", padding: "32px 24px", border: p.popular ? `2px solid ${T.primary}` : `1px solid ${T.border}`,
                ...
                {p.features.map(f => (
                  <div key={f} style={{ display: "flex", gap: "10px", marginBottom: "10px", fontSize: "14px", color: T.text, alignItems: "flex-start" }}>
                    <span style={{ color: T.accent, fontSize: "16px", flexShrink: 0, marginTop: "0px" }}>✓</span>{f}
                  </div>
                ))}
          ...
          <div style={{ textAlign: "center", marginTop: "32px", color: T.muted, fontSize: "13px" }}>🔒 Secured by Stripe · No hidden fees · Cancel anytime</div>
        </div>
      </div>
    );
  }
  ...
  const HomeTab = () => (
    <div className="fadeUp">
      ...
      <div style={{ background: `linear-gradient(135deg, #1e1030, #2a1520)`, borderRadius: "24px", padding: "32px", marginBottom: "20px", border: `1px solid ${T.primary}44`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ fontFamily: T.display, fontSize: "28px", fontWeight: "900", color: T.text, margin: "12px 0 6px", letterSpacing: "-0.5px" }}>
          Hey, {user?.name}! 👋
        </div>
        <div style={{ color: T.muted, fontSize: "14px", marginBottom: "24px", lineHeight: "1.6" }}>
          Ready to level up today? Pick where you want to start.
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", marginBottom: "20px" }}>
        {[
          ...
          <Card key={item.title} onClick={item.action} style={{ cursor: "pointer", padding: "20px" }}>
          ...
          <Card style={{ marginBottom: "14px" }}>
            ...
            {roadmap.weeks?.map((w, i) => (
              <div key={i} style={{ display: "flex", gap: "14px", marginBottom: "16px", alignItems: "flex-start" }}>
                <div style={{ background: `linear-gradient(135deg, ${T.primary}, #ff9a4a)`, borderRadius: "12px", padding: "6px 12px", fontSize: "12px", fontWeight: "800", color: "#fff", whiteSpace: "nowrap", flexShrink: 0, boxShadow: `0 4px 12px ${T.primaryGlow}` }}>{w.week}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: "700", color: T.text, fontSize: "14px", marginBottom: "6px" }}>{w.topic}</div>
                  {w.tasks?.map((t, j) => (
                    <div key={j} style={{ display: "flex", gap: "8px", fontSize: "13px", color: T.muted, marginBottom: "4px", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", marginBottom: "14px" }}>
            <Card>
              ...
              {roadmap.skills?.map((s, i) => (
                <div key={i} style={{ display: "flex", gap: "8px", marginBottom: "8px", fontSize: "13px", color: T.muted, alignItems: "center" }}>
                  <span style={{ color: T.primary, fontWeight: "800" }}>✓</span>{s}
                </div>
              ))}
            </Card>
            <Card>
              <div style={{ fontWeight: "800", color: T.text, marginBottom: "14px", fontSize: "15px" }}>📁 Projects to Build</div>
      ...
      {cards.length === 0 ? (
        <Card>
          ...
          <TextArea value={notes} onChange={e => setNotes(e.target.value)}
            placeholder="e.g. The water cycle involves evaporation, condensation and precipitation. Evaporation is when water turns into vapor due to heat from the sun..." />
        ...
        <>
          ...
          <div onClick={() => setFlipped(!flipped)} style={{
            ...
            <div style={{ position: "absolute", top: "16px", right: "18px", fontSize: "12px", color: T.muted, fontWeight: "600" }}>
              {cardIdx + 1}/{cards.length}
            </div>
            <Badge color={flipped ? T.accent : T.primary} style={{ marginBottom: "16px" }}>
              {flipped ? "💡 Answer" : "❓ Question"}
            </Badge>
            <div style={{ fontFamily: T.display, fontSize: "20px", lineHeight: "1.5", color: T.text, fontStyle: flipped ? "italic" : "normal" }}>
          ...
          <div style={{ display: "flex", gap: "12px", marginBottom: "14px" }}>
            <Btn onClick={() => { setCardIdx(i => Math.max(0, i - 1)); setFlipped(false); }} variant="muted"
              disabled={cardIdx === 0} style={{ flex: 1 }}>← Prev</Btn>
            <Btn onClick={() => { setCardIdx(i => Math.min(cards.length - 1, i + 1)); setFlipped(false); }}
              disabled={cardIdx === cards.length - 1} style={{ flex: 1 }}>Next →</Btn>
          </div>
      ...
      {quiz.length === 0 ? (
        ...
        <Card style={{ textAlign: "center", padding: "48px 28px" }}>
          ...
          <div style={{ background: T.surface, borderRadius: "16px", padding: "20px", marginBottom: "28px" }}>
            <div style={{ height: "10px", background: T.border, borderRadius: "8px", overflow: "hidden", marginBottom: "10px" }}>
              <div style={{ height: "100%", width: `${(score / quiz.length) * 100}%`, background: score >= 4 ? `linear-gradient(90deg, ${T.accent}, #22d3c0)` : score >= 3 ? `linear-gradient(90deg, ${T.primary}, #ff9a4a)` : `linear-gradient(90deg, ${T.danger}, #ff7a8a)`, borderRadius: "8px", transition: "width 0.8s ease" }} />
            </div>
        ...
        <>
          ...
          <Card style={{ marginBottom: "14px", border: `1px solid ${T.primary}44` }}>
            <Badge color={T.primary} style={{ marginBottom: "12px", display: "inline-block" }}>Question {qIdx + 1}</Badge>
            <div style={{ fontFamily: T.display, fontSize: "19px", color: T.text, lineHeight: "1.5", marginTop: "8px" }}>{quiz[qIdx].question}</div>
          </Card>
  ...
  const ChatTab = () => (
    <div className="fadeUp" style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 180px)" }}>
      <div style={{ marginBottom: "16px" }}>
        ...
        <div style={{ color: T.muted, fontSize: "14px" }}>Ask anything — any subject, any level, any language</div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: T.surface, borderRadius: "20px", padding: "16px", marginBottom: "12px", border: `1px solid ${T.border}` }}>
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", marginBottom: "14px" }}>
            {m.role === "ai" && (
              <div style={{ width: "34px", height: "34px", borderRadius: "12px", background: `linear-gradient(135deg, ${T.primary}, #ff9a4a)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", marginRight: "8px", flexShrink: 0, boxShadow: `0 4px 12px ${T.primaryGlow}` }}>✨</div>
            )}
            <div style={{
              ...
              whiteSpace: "pre-wrap", boxShadow: m.role === "user" ? `0 4px 16px ${T.primaryGlow}` : "none"
            }}>{m.text}</div>
          </div>
        ))}
        {chatLoad && (
    ...
    <div style={{ fontFamily: T.font, background: T.bg, color: T.text, minHeight: "100vh" }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: `${T.surface}ee`, backdropFilter: "blur(20px)", borderBottom: `1px solid ${T.border}`, padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div style={{ width: "36px", height: "36px", borderRadius: "12px", background: `linear-gradient(135deg, ${T.primary}, #ff9a4a)`, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "800", fontSize: "15px", boxShadow: `0 4px 12px ${T.primaryGlow}` }}>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, background: `${T.surface}f0`, backdropFilter: "blur(20px)", borderTop: `1px solid ${T.border}`, display: "flex", justifyContent: "space-around", padding: "10px 0 18px" }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            fontWeight: tab === t.id ? "700" : "500", fontSize: "11px", fontFamily: T.font,
            transition: "all 0.2s", position: "relative"
          }}>
            {tab === t.id && <div style={{ position: "absolute", top: "-10px", left: "50%", transform: "translateX(-50%)", width: "24px", height: "3px", background: T.primary, borderRadius: "3px", boxShadow: `0 0 8px ${T.primary}` }} />}
            <span style={{ fontSize: "22px", filter: tab === t.id ? `drop-shadow(0 0 6px ${T.primary})` : "none" }}>{t.icon}</span>
            {t.label}
          </button>
        ))}
      </div>
